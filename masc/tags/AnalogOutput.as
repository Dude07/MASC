/*                            ------------------------------------------ЕЩЁ НЕ ДОДЕЛАНО------------------------------------------------
MASC 2018. AnalogOutput
топик для публикации данных выглядит так:	MASC/tags/PLC/AQ/num	{"fiz":###,"SP":###,"SPcor":###, "err":#}
					 настроек сигнала:		MASC/sets/PLC/AQ/num	{"eu":@,"name":@,"fiz_min":###,"fiz_max":###,"cod_min":#####,"cod_max":#####,"mA_min":##,"mA_max":##,"filtr":##}
...
aq:AnalogOutput
  {
	  ...
	  function AnalogOutput  Функция которая выполняется при инициализации сигнала.
...
package masc.tags {
	...
	public class AnalogOutput {
		public var fiz,fiz_notF,fizOld,SP,SPold,SPcor,fiz_min, fiz_max,cod,cod_min,cod_max,mA,mltp:Number;
		public var num,err,filtr,mA_min,mA_max,noSub:int;
		public var type:int;
		public var name:String="Аналоговый вход"
		public var src,tag,dst,teg,eu:String;
		...
		public function addProp(nam:String,val:*):void{
			switch (nam){
				...
				default: trace("Свойство AQ."+nam+" не назначено. Нет в перечне функции addProp");  break;
			}
		}
		...
		public function getProp(nam:String):*{
			switch (nam){
				...
				default: trace("Свойство AQ."+nam+" не возвращено. Нет в перечне функции getProp"); return null; break;
			}
			return null;
		}
	}
