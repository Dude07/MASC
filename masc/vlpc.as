...
function setUpVPLC():void{ //распаковка нового проекта
	...
	for each (var table in projectXML..ns::Tables..ns::table){
		...
		for each(var pair in table..ns::pair){
			resDP.push({ins:Number(pair.@ins),out:Number(pair.@out)});
		}
	...
	for each (var di in projectXML..ns::PLC..ns::di) {
		var discretInput:DiscretInput=new DiscretInput();
		for each (par in di.attributes()) discretInput.addProp(par.name(),par.valueOf());
		for each (reg in di..ns::reg){
			regulator=new Regulator();
			for each (par in reg.attributes()) regulator.addProp(par.name(),par.valueOf());
			for each (cor in reg..ns::cor){
				correction=new Correction();
				for each (par in cor) correction.addProp(par.name(),par.valueOf());
				regulator.cors[correction.num]=correction;
			}
			discretInput.regs[regulator.num]=regulator;
		}
		DI.push(discretInput);
	}
	for each (var dq in projectXML..ns::PLC..ns::dq) {
		var discretOutput:DiscretOutput=new DiscretOutput();
		for each (par in dq.attributes()) discretInput.addProp(par.name(),par.valueOf());
		for each (reg in dq..ns::reg){
			regulator=new Regulator();
			for each (par in reg.attributes()) regulator.addProp(par.name(),par.valueOf());
			for each (cor in reg..ns::cor){
				correction=new Correction();
			...
			discretOutput.regs[regulator.num]=regulator;
		}
		DQ.push(discretOutput);
	}//manageDiscretOutput("init",[int(dq.@type),int(dq.@num),dq.@name,dq.@src,dq.@dst,dq.@filtr]);
...
function searchData(topic:String, object:Object):void{
	...
	for (var prop in object){
		...
		if (resNum>=0){
			...
			switch (resType) {
				case "AI": if (resNum<AI.length) {processAI(AI[resNum]); publishAI(AI[resNum]); if (AI[resNum].pub==true) updateEngine=true; } break;
				case "DI": if (resNum<DI.length) {processDI(DI[resNum]); publishDI(DI[resNum]); if (DI[resNum].pub==true) updateEngine=true; } break;
				case "DQ": if (resNum<DQ.length) {processDQ(DQ[resNum]); publishDQ(DQ[resNum]); if (DQ[resNum].pub==true) updateEngine=true; } break;
			}
		}
	...
	for each(kotelnaya in kotelnie){
		...
		if (kotelnaya.algorithm.SP!=undefined) {
			var val:Number=Number(getValue(kotelnaya.algorithm.SP));
			if (kotelnaya.SP!=val) { kotelnaya.SP=val; kotelnaya.process=true;}
		}
		if (kotelnaya.algorithm.fiz!=undefined) { 
			val=Number(getValue(kotelnaya.algorithm.fiz));
			if (kotelnaya.fiz!=val) { kotelnaya.fiz=val; kotelnaya.process=true;}
		}
		...
		//if (kotelnaya.process) processKotelnaya(kotelnaya); //алгоритм управления котельными как переделаю тот пшик с вызовом по таймеру, верну опять сюда
	}
	iter+=1;
	iter_txt.text=iter.toString();    
	updateClients();
...
var regPLC:RegExp = /(AI|AQ|DI|DQ|DD|DA)\d*/ig;
function getValue(st:String,val:String=""):*{
	var ret:*=null; var arr:Array; var par:String="";
	if (val!=""){
		par=val;
	} else {
		arr=st.split(":");
		st=arr[0];
		if (arr.length>1) {arr.reverse(); par=arr[0];} else par="RAW";
	}
	if (st.indexOf("PLC")>=0){
		var arr1:Array=st.split("/");
		arr1.reverse();
		var nn:int=int(arr1[0]);
		var typ:String=arr1[1];
		if (par=="RAW") par="fiz";
		
		switch (typ.toString()) {
			case "AI": if (nn<AI.length) {ret=AI[nn].getProp(par)} break;
			case "DI": if (nn<DI.length) {ret=DI[nn].getProp(par)} break;
			case "DQ": if (nn<DQ.length) {ret=DQ[nn].getProp(par)} break;
		}
		//trace("Got data type "+typ,nn,par+" = "+ret);
		return ret;
	}
	
	
	if (mainData[st]!=undefined){
		if (par=="RAW") {
			ret=mainData[st];
		} else {
			var obj:Object=mainData[st];
			if (obj[st]!=undefined) ret=obj[st];
		}
	}
	return ret;
}
//----------------------Временный вариант обработки котельных по таймерау---------------------------------//
var tm:Timer=new Timer(1000);
function timerProcessKotelnaya(e:TimerEvent):void{
	for each(var kotelnaya in kotelnie){
		processKotelnaya(kotelnaya);
	}
}
tm.addEventListener(TimerEvent.TIMER,timerProcessKotelnaya);
tm.start();
//--------------------------------------------------------------------------------------------------------//

function addAnalogInput(analogInput:AnalogInput):void{
	for each (var ai in AI) if (ai.num>=analogInput.num) analogInput.num=ai.num+1;
	AI.push(analogInput);
}
...
}//Фунция добавления нового элемента в PLC основного XML проекта
...
function updateFunc(object:Object):void {
	...
	switch (object.f){
		case "AI": trace("Adding new AI"); break; //manageAnalogInput(object.a,(object.p).toString().split(","))
	}
}
